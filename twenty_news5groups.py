...
def download_20news5groups(target_dir, cache_path):
    ...
    cache = dict(train=load_files_supernews(train_path, encoding='latin1'),
                 test=load_files_supernews(test_path, encoding='latin1'))
...
def fetch_20news5groups(data_home=None, subset='train', categories=None,
    ...
    if cache is None:
        if download_if_missing:
            cache = download_20news5groups(target_dir=twenty_home,
...
def load_files_supernews(container_path, description=None, categories=None,
               load_content=True, shuffle=False, encoding=None,
    ...
    target_names=['computers','recreation','science','politics','religion'] 
    for label, folder in enumerate(folders):
        if (label>=1 and label <=6):
            newlabel=0
        elif (label>=7 and label <=10):
            newlabel=1
        elif (label>=11 and label <=14):
            newlabel=2
        elif (label>=16 and label <=18):
            newlabel=3
        else:
            newlabel=4
        #target_names.append(categories[newlabel])
        folder_path = join(container_path, folder)
        documents = [join(folder_path, d)
                     for d in sorted(listdir(folder_path))]
        target.extend(len(documents) * [newlabel])
    ...
    #for targetnames in target_names:
        #print targetnames
